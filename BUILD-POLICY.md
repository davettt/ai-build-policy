...
| 2.5 | 2026-07-28 | Portfolio consistency baselines. Prettier: `semi`, `singleQuote`, `tabWidth`, `printWidth` must match the house baseline; `trailingComma`, `arrowParens`, `plugins` and `endOfLine` remain per-project. Testing: a stub `test:smoke` (`echo`, `exit 0`, `npm run build`) FAILs, and `tests/smoke.js` plus `tests/harness.js` are required for any project with a server. Optional `test:unit` tier added for logic requiring no server. Action: 10 projects need a config change and a one-time reformat; 4 need real smoke/integration tests. |
