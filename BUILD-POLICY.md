...
| Gates re-run on GitHub: static checks, SAST, audit, licenses, **build, and every test tier** (+ gitleaks-action). `deps:check` and CodeRabbit stay local — the first resolves a sibling repo absent from a CI checkout, the second is covered by CodeRabbit's own GitHub app on PRs | GitHub Actions CI on every push/PR — **the durable evidence record** |
...
|---|---|---|
| 2.6.2 | 2026-08-01 | CI ran no build and no tests. The shared `ci.yml` covered lint/format/type-check/SAST/audit/licenses/secrets only, so the test suites — 46 tests in one new project alone — existed solely on the developer's machine, while this document called CI "the same gates re-run on GitHub, the durable evidence record". Template now runs `build`, `test:unit`, `test:smoke`, `test:integration`, each `--if-present` so a project runs the tiers it defines; the smoke/integration isolation rule (own port, temp data dir) from 2.5 is what makes them runnable on a bare runner. `deps:check` deliberately stays out — it executes `../build-policy/scripts/check-allowlist.js`, a sibling repo that does not exist in a CI checkout — and CodeRabbit stays out, being covered by its GitHub app on PRs; both noted in the template so the omissions are not mistaken for oversights. The workflow table no longer claims parity it did not have. All 10 projects with a `ci.yml` now show template drift: sync with `cp ../build-policy/templates/ci.yml .github/workflows/ci.yml`. |
| 2.6.1 | 2026-08-01 | Second door into the same hole, closed. CodeRabbit exits **0** with `"status":"review_skipped","message":"No changes detected"` whenever the tree is clean and the branch equals its base — the normal state right after a commit — and `cmdGates` judged gates by exit code alone, so `policy gates` recorded "CodeRabbit review: pass" into the marker for code the reviewer never opened. Any project committing straight to main (which this policy prescribes) could pass full gates on an unreviewed tree; found when a new project's 12k-line scaffold went in via a root commit and every later gates run reported a clean review of nothing. `gates` now parses the `--agent` output it already requests and FAILs a review that examined nothing, printing how to review already-committed work (`--base-commit <sha>`, or a PR / scratch-repo review for a root commit, which has no predecessor to diff). Verified both ways: skipped review FAILs, genuine review still passes. |
| 2.6 | 2026-08-01 | Review gate was skipping the code most worth reviewing, and new repos could not run it at all. `STANDARD_SCRIPTS.review` was `coderabbit review --agent`, whose default scope is TRACKED changes — and gates run before staging, so every brand-new file went through the review gate unseen, in all 11 projects that have the script. Now `--include-untracked` (verified against the CLI: an untracked file is reported as `reviewedFiles` with the flag, ignored without it); `check` FAILs any review script missing it. New-repo deadlock diagnosed rather than worked around: CodeRabbit fails at `gitService.getBranchInfo` (it runs `git rev-parse --abbrev-ref HEAD`), so it needs a commit — any commit, including `--allow-empty` — not a diff base, and with no remote it also needs `git config coderabbit.baseBranch <branch>`. `verify-marker` now waives itself when there is no HEAD, so the root commit is allowed and `--no-verify` is never needed; gating resumes on the very next commit. `gates` preflights both conditions and prints the exact command, instead of surfacing raw CLI JSON mid-run. |
| 2.5.1 | 2026-07-29 | Gates marker fixed at the source of its own contract. `changedFiles()` used `git status --porcelain`, which collapses an untracked directory to a single `?? dir/` entry but lists its files individually once staged — so the file list, and `diffHash` with it, changed on `git add`, exactly what the function documents itself as never doing. Any project adding a directory (a new `tests/`, say) therefore burned a full gates re-run, CodeRabbit included, on the first `git add`. The sharper edge: `readFile()` on a directory path returns `""` (EISDIR), so a new directory's contents hashed as nothing and edits inside it never invalidated a marker — `verify-ready` and the Stop hook could report ready on code gates never saw (the commit itself stayed blocked, since `git add` shifted the hash and pre-commit `verify-marker` caught it). Fixed with `-uall`, which lists untracked files individually, is byte-identical staged or unstaged, and still honours `.gitignore`; preferred over expanding entries via `git ls-files` per directory, which would mean a subprocess per entry and a path interpolated into a shell string. No check result changes anywhere in the portfolio. |
| 2.5 | 2026-07-28 | Portfolio consistency baselines, after a survey found the gates were green on drift they never looked at. **Prettier:** 24 configs all set `singleQuote` (the only key `check` verified) and diverged on everything else — `semi` split 18/5, `printWidth` 100/80, `tabWidth` 2/4. Baseline (not template equality) now enforced: `semi`, `singleQuote`, `tabWidth`, `printWidth` must match; `trailingComma`, `arrowParens`, `plugins`, `endOfLine` stay per-project. These four govern line shape, so deviation makes cross-project formatting destructive — a `prettier --write` borrowing a neighbouring config reflowed 700 lines of `policy.js` during this very session. Flags 10 projects, each one mechanical reformat. **Testing:** `check` accepted any script named `test:smoke`, so `echo 'No smoke tests configured'` (one dashboard app) and `npm run build` (one mail client) passed while testing nothing, and inline `node -e "fetch(...)"` (two tracker apps) only passed when a dev server happened to be live — a test with production data in reach. Stubs now FAIL, and `tests/smoke.js` + `tests/harness.js` are both required for any project with a server. **Tiers:** optional `test:unit` sanctioned for logic needing no server (one app had bent `test:smoke` to hold unit tests for want of a home); smoke/integration/E2E unchanged. |
| 2.4.3 | 2026-07-28 | Policy-doc version self-consistency enforced: the `**Version:**` header had stayed at 2.4 while 2.4.1 and 2.4.2 history rows were appended, so commit messages and any citation of the policy version disagreed with the document. `mirror` could not catch it — it only compared the private header to the public one, and both were equally stale. New `auditPolicyDocVersions` gate (run by `check` inside the policy repo, and by `mirror` against BOTH copies before the cross-copy comparison): header must equal the highest version in the history table, the table must read newest-first, and `project-standards.md` must carry the same version. Headers corrected to 2.4.2 and the 2.4-series rows reordered newest-first. Separately: the policy repo itself had no `.prettierrc` — the exemption `check` grants documentation-only projects (no package.json) meant any prettier run against `scripts/` borrowed a foreign project config and reflowed the whole file. `templates/prettierrc` copied in (both copies) and `scripts/` formatted to it, so the house config now governs the enforcement code too. |
