...
Another shipped DMGs whose banner could never fire: the app is served from localhost, so reading `version.json` is a cross-origin request, and with no `Access-Control-Allow-Origin` header the browser blocks it before any app code runs. Nothing caught it because every check reads the repo, and that header is served by a Cloudflare dashboard rule that no file in the repo describes. A correct banner and a working banner are different claims, and only one can be verified by reading code. `verify-ready --release` now fetches the live endpoint and asserts both a 200 and the CORS header.

Note the site-wide CORS rule is dashboard configuration, outside version control: nothing in the repo guarantees it stays, and removing it would break every app's banner at once, silently. The release check is the only thing that would notice.
- **Anything reached from a link must be dismissible.** Both handlers test the server origin and they must agree. An unconditional `shell.openExternal` in `setWindowOpenHandler` reads as correct and is not: the app's own pages are served over the local server, so a `target="_blank"` link to a route like `/api/licenses` is handed to the browser as `http://127.0.0.1:<random port>/api/licenses` — raw text on a localhost port, dead once the app quits. App content belongs in the app.

  Allowing it back in is only half the fix. `action: 'allow'` on its own, or a plain same-origin link that `will-navigate` lets through, replaces the app UI with a text file that has no back button and no close control, and the user is stuck. Same-origin windows need `overrideBrowserWindowOptions` so they are real, closable windows, or the content is rendered in the app behind a close control. `check` FAILs a `setWindowOpenHandler` whose body has no origin test.
...
- **The gate is conditional on source, and unconditional at release.** **A review is not repeated for source it has already read.** The gates marker hashes the whole tree, so adding the CHANGELOG entry the Stop hook requires invalidates it, and the next run re-reviewed the same code plus one prose line at full cost against a small rolling allowance. The review decision is now keyed on a hash of the changed source alone: an unchanged codebase is not re-reviewed however many times the changelog, docs or config move. A carried-forward review is still recorded in the marker as a pass, because the code was genuinely reviewed, so `verify-ready --release` is neither weakened nor tripped by it. It carries forward only when the previous marker records the gate AND the source is byte-identical, so it can shorten the path and never skip a first review. The CLI allowance is a few reviews per rolling window, so `policy gates` skips the review when the diff contains only config, docs, or changelog files — there is nothing in it for a code reviewer to read, and spending the review there leaves nothing for the next real change. Any source file in the diff brings it back, `gates --with-review` forces it, and `verify-ready --release` FAILs unless the marker records a CodeRabbit pass, so nothing ships unreviewed. A clean tree is not treated as source-free: the gate still runs, which is what keeps the review_skipped FAIL below catching already-committed work.
...
**Content presented for approval is verified against the file, not recalled.** A session once presented a fourteen-row table of in-app tips for review, reconstructed from memory after a context summarisation. Thirteen of the fourteen did not exist. The danger is not the error, it is that nothing looked wrong: an approver would have signed off content that is not in the product, and the transcript would have shown a confident, well-formatted table.

Checking the process does not work here. The obvious rule, "was the file read before it was described", would have missed this exactly: the file *had* been read, and quoted accurately at the time, two hundred messages earlier. The summarisation destroyed the knowledge while leaving the read in the transcript, so the evidence of diligence outlived the diligence.

So the Stop hook verifies the claim. Identifiers cited in the closing message are compared against the identifiers that actually exist in the project's JSON data files, and the turn is blocked when the message is demonstrably about that dataset (at least one real match) yet cites two or more that do not exist. Measured against the transcript of the real incident: the accurate table scored 14 real / 0 absent, the fabricated one 1 real / 13 absent.

The rule for a session is simpler than the check: read the file in the same turn you describe its contents. A read from earlier in the session is not evidence you still know what it says.

**Compliance is a gate, not a report.** It was advisory: `policy check` ran in the SessionStart hook and nowhere else — not in the Stop hook, not in pre-commit, not in gates — and it printed rather than blocked. So a project could fail every structural rule and still gate, commit, sign and ship, with the only thing in the gap being a line of prose telling a human to fix FAIL items first. That is the failure mode this policy exists to remove, sitting at its centre.

Worse than "checked once per session": anything introduced *after* the session started was not checked until the next one. In a single sitting an app can add a CSP that kills its update banner, pass every other gate, satisfy `verify-ready`, commit, build a signed DMG and ship — with compliance never consulted.

It runs as the **first** gate, before tests and the build, because a structural gap found there is fixed while the fix is cheap. Binding it at release instead would surface it after testing and force a retest.

**There is no mechanism to accept a failure as known debt.** A gate that lets you record failures as acceptable is a report with extra steps, and an escape hatch reachable by whoever was just stopped is precisely what this policy exists to remove. Compliance is met, or the gates do not run. That is affordable because such a backlog is mostly mechanical: scaffolding, missing devDependencies, template drift and one-line script flags, with a minority needing real thought — and those are the ones worth being stopped for.
...
The work is split because release dates cost a network call per package (`npm view <pkg> time`), which cannot sit in a session-start check. `health` measures which minor/patch updates have been available longer than the grace period (`staleness.depsStaleDays`, 30 days) and records them; `check` fails on that recorded verdict at no cost, every time gates run. Majors are untouched and keep going through `policy upgrade` and its decision record: being behind a major is a decision, being behind a patch for months is neglect.

A grace period rather than a count of reports. Counting ties enforcement to how often a project happens to be opened, so the least-visited repos — the stalest — would accrue counts slowest, which is backwards. Wall-clock time does not care how often you visit.
